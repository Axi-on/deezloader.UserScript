...
function translateCircumstring(data) {
  return data.replace(/\^(\w+)/g, (full, part) =>
    translate[part] ? translate[part] : full
  );
}
...
if (GM_info.scriptHandler === "Greasemonkey") {
  if (!localStorage.gmFetchWarningShown) {
    ...
    alert(
      "We cannot safely replace `fetch()' in GreaseMonkey. If troubles starts, replace GreaseMonkey with ViolentMonkey or TamperMonkey. This message will not be shown anymore"
    );
  }
} else {
  ...
  unsafeWindow.fetch = (input, init = {}) => {
    ...
    if (
      input.startsWith(
        "https://www.deezer.com/ajax/gw-light.php?method=log.listen"
      )
    )
      return;
...
function trackRecurseLookup(key) {
  if (key !== null && typeof key === "object") {
    if (key.SNG_ID) trackDB[key.SNG_ID] = key;
    else if (key.forEach) key.forEach(trackRecurseLookup);
    else Object.keys(key).forEach(subKey => trackRecurseLookup(key[subKey]));
  }
}
...
function niceXhr(config) {
  const data = {
    ...
    url: config.url,
    onload: e => {
      if (200 === e.status) config.onSuccess(e.response);
      else console.warn("Error with getting data from", e.finalUrl);
      config.anyway();
    },
  ...
  else GM_xmlhttpRequest(data);
}

// WORKER
function createWorker(code) {
  const blobURL = URL.createObjectURL(
    new Blob(["(", code.toString(), ")()"], { type: "text/javascript" })
  );
...
const mainWk = createWorker(function() {
  ...
  Blowfish.prototype = {
    ...
    block32toNum: function(block32) {
      return this.fixNegative(
        (block32[0] << 24) | (block32[1] << 16) | (block32[2] << 8) | block32[3]
      );
    },
    xor: function(a, b) {
    ...
    split64by32: function(block64) {
      return [
        this.block32toNum(block64.subarray(0, 4)),
        this.block32toNum(block64.subarray(4, 8))
      ];
    },
    decryptCBC: function(data, iv) {
      ...
      for (let i = 0; i < blocks; i++) {
        ...
        if (block.length < 8)
          throw new Error(
            "BF: ciphertext too short (must be multiple of 8 bytes)"
          );
  ...
  const ID3Writer = (() => {
    ...
    function n(e) {
      let r = new Uint8Array(2 * e.length);
    ...
    function i(e) {
      ...
      if (87 === e[8] && 69 === e[9] && 66 === e[10] && 80 === e[11])
        return "image/webp";
      let t = 73 === e[0] && 73 === e[1] && 42 === e[2] && 0 === e[3],
    ...
    return (function() {
      function t(r) {
        if ((e(this, t), !(r && "object" == typeof r && "byteLength" in r)))
          throw new Error(
      ...
      return (
        (t.prototype._setIntegerFrame = function(e, t) {
          let r = parseInt(t, 10);
          this.frames.push({ name: e, value: r, size: u(r.toString().length) });
        }),
        (t.prototype._setStringFrame = function(e, t) {
          let r = t.toString();
          this.frames.push({ name: e, value: r, size: f(r.length) });
        }),
        (t.prototype._setPictureFrame = function(e, t, r, n) {
          let a = i(new Uint8Array(t)),
            s = r.toString();
          if (!a) throw new Error("Unknown picture MIME type");
          r || (n = !1),
            this.frames.push({
              name: "APIC",
              value: t,
        ...
        (t.prototype._setUrlLinkFrame = function(e, t) {
          let r = t.toString();
          this.frames.push({ name: e, value: r, size: m(r.length) });
        }),
        (t.prototype.setFrame = function(e, t) {
          switch (e) {
            ...
            case "TKEY":
              if (!/^([A-G][#b]?m?|o)$/.test(t))
                throw new Error(
                  e + " frame value should be like Dbm, C#, B or o"
                );
              this._setStringFrame(e, t);
              break;
            case "WCOM":
        ...
        (t.prototype.removeTag = function() {
          if (!(this.arrayBuffer.byteLength < 10)) {
            let e = new Uint8Array(this.arrayBuffer),
              t = e[3],
          ...
          return (
            ...
            this.frames.forEach(function(a) {
              switch (
                ...
                case "WPAY":
                case "WPUB":
                  (f = r(a.value)), c.set(f, u), (u += f.length);
                  break;
                case "TPE1":
                case "TCOM":
                case "TCON":
                ...
                case "TLEN":
                case "TYER":
                  u++, (f = r(a.value)), c.set(f, u), (u += f.length);
                  break;
                case "APIC":
  ...
  const FLACMetadataEditor = (() => {
    ...
    class VorbisCommentPacket {
      _addComment(t) {
        const e = t.split("=")[1];
        return (
          (t = t.split("=")[0].toUpperCase()),
          this.hasOwnProperty(t) || (this[t] = new VorbisComment()),
          this[t].some(t => t === e) || this[t].push(e.toString()),
          this
        );
      }
      toStringArray() {
    ...
    class FLACMetadataBlock {
      ...
      get serializedSize() {
        switch (this.blockType) {
          ...
          case "APPLICATION":
            return 4 + this.data.applicationData.length;
          case "SEEKTABLE":
            return 18 * this.data.points.length;
          case "VORBIS_COMMENT":
            const t = this.data.comments
              .toStringArray()
              .reduce((t, e) => t + 4 + e.toUTF8().length, 0);
            return 4 + this.data.vendorString.length + 4 + t;
          case "CUESHEET":
            return 0;
          case "PICTURE":
            return (
              8 +
              this.data.MIMEType.toUTF8().length +
              4 +
              this.data.description.toUTF8().length +
              4 +
              4 +
              4 +
              4 +
    ...
    return class {
      ...
      constructor(t) {
        ...
        return (
          ...
          (String.prototype.toUTF8 = function(t = null) {
            return new TextEncoder().encode(t || this);
          }),
          this._parseMetadata(),
          this
        );
      }
      _getBytesAsNumber(t, e = 0, s = t.length - e) {
        return Array.from(t.subarray(e, e + s)).reduce(
          (t, e) => (t = 256 * t + e),
          0
        );
      }
      _getBytesAsNumberLittleEndian(t, e = 0, s = t.length - e) {
        return Array.from(t.subarray(e, e + s)).reduceRight(
          (t, e) => (t = 256 * t + e),
          0
        );
      }
      _getBytesAsHexString(t, e = 0, s = t.length - e) {
        return Array.from(t.subarray(e, e + s))
          .map(t => (t >> 4).toString(16) + (15 & t).toString(16))
          .join("");
      }
      _getBytesAsUTF8String(t, e = 0, s = t.length - e) {
        return new TextDecoder().decode(t.subarray(e, e + s));
      }
      _getBlockType(t) {
        switch (t) {
          case 0:
      ...
      _uint32ToUint8Array(t) {
        return [(t >>> 24) & 255, (t >>> 16) & 255, (t >>> 8) & 255, 255 & t];
      }
      _uint24ToUint8Array(t) {
        return [(t >>> 16) & 255, (t >>> 8) & 255, 255 & t];
      }
      _uint16ToUint8Array(t) {
        return [(t >>> 8) & 255, 255 & t];
      }
      _hexStringToUint8Array(t) {
        return t
          .replace(/(\w\w)/g, "$1,")
          .slice(0, -1)
          .split(",")
          .map(t => (parseInt(t[0], 16) << 4) + parseInt(t[1], 16));
      }
      get _vorbisComment() {
        const t = this.metadata.blocks.find(
          t => "VORBIS_COMMENT" === t.blockType
        );
        if (t) return t.data;
      }
      addComment(t, e = null) {
        if (t) {
          ...
          (t = t.toUpperCase()),
            this._vorbisComment.comments.hasOwnProperty(t) ||
              (this._vorbisComment.comments[t] = new VorbisComment()),
            this._vorbisComment.comments[t].find(t => t === e) ||
              this._vorbisComment.comments[t].push(e.toString());
        }
        return this;
      }
      removeComment(t = null, e = null) {
        return (
          t
            ? ((t = t.toUpperCase()),
              e
                ? ((e = e.toString()),
                  this.hasOwnProperty(t) &&
                    (this._vorbisComment.comments[
                      t
                    ] = this._vorbisComment.comments[t].filter(t => t !== e)))
                : delete this._vorbisComment.comments[t])
            : Object.keys(this._vorbisComment.comments).forEach(
                t => delete this._vorbisComment.comments[t]
              ),
          this
        );
      }
      getComment(t) {
        return this._vorbisComment.comments[t.toUpperCase()];
      }
      addPicture(t) {
        if (
          !(
          ...
          throw new Error(
            'Field "data" should be an instance of ArrayBuffer or Buffer'
          );
        ...
        const e = {
          ...
          s = new FLACMetadataBlock();
        (s.blockTypeNubmer = 6), (s.blockType = "PICTURE");
        for (let r in e) t[r] ? (s.data[r] = t[r]) : (s.data[r] = e[r]);
        const r = this.metadata.blocks;
        let a = r.length;
        return (
          "PADDING" === r[r.length - 1].blockType && a--,
          r.splice(a, 0, s),
          (this.metadata.blocks = r),
          this
        );
      }
      _serializeMetadataBlock(t) {
        const e = new Uint8Array(t.serializedSize),
          s = t.data;
        let r = 0;
        switch (t.blockType) {
          case "STREAMINFO":
            e.set(this._uint16ToUint8Array(s.minBlockSize)),
              (r += 2),
              e.set(this._uint16ToUint8Array(s.maxBlockSize), r),
              (r += 2),
              e.set(this._uint24ToUint8Array(s.minFrameSize), r),
              (r += 3),
              e.set(this._uint24ToUint8Array(s.maxFrameSize), r),
              (r += 3),
              e.set(
                this._uint24ToUint8Array(
                  (s.sampleRate << 4) +
                    ((s.numberOfChannels - 1) << 1) +
                    ((s.bitsPerSample - 1) >> 4)
                ),
                r
              ),
              (e[(r += 3)] =
                (((s.bitsPerSample - 1) & 15) << 4) +
                (15 & Math.trunc(s.totalSamples / Math.pow(2, 32)))),
              (r += 1),
              e.set(this._uint32ToUint8Array(s.totalSamples), r),
              (r += 4),
              e.set(this._hexStringToUint8Array(s.rawMD5), r);
            break;
          case "PADDING":
            break;
          case "APPLICATION":
            e.set(s.applicationID.toUTF8()),
              (r += 4),
              e.set(s.applicationData, r);
            break;
          case "SEEKTABLE":
            s.points.forEach(t => {
              e.set(this._hexStringToUint8Array(t.sampleNumber), r),
                e.set(this._hexStringToUint8Array(t.offset), r + 8),
                e.set(this._hexStringToUint8Array(t.numberOfSamples), r + 16),
                (r += 18);
            });
            break;
          case "VORBIS_COMMENT":
            e.set(
              this._uint32ToUint8Array(
                s.vendorString.toUTF8().length
              ).reverse(),
              r
            ),
              (r += 4),
              e.set(s.vendorString.toUTF8(), r),
              (r += s.vendorString.toUTF8().length);
            const a = s.comments.toStringArray();
            e.set(this._uint32ToUint8Array(a.length).reverse(), r),
              (r += 4),
              a.forEach(t => {
                e.set(this._uint32ToUint8Array(t.toUTF8().length).reverse(), r),
                  (r += 4),
                  e.set(t.toUTF8(), r),
                  (r += t.toUTF8().length);
              });
          ...
          case "CUESHEET":
            break;
          case "PICTURE":
            e.set(this._uint32ToUint8Array(s.APICtype)),
              (r += 4),
              e.set(this._uint32ToUint8Array(s.MIMEType.toUTF8().length), r),
              (r += 4),
              e.set(s.MIMEType.toUTF8(), r),
              (r += s.MIMEType.toUTF8().length),
              e.set(this._uint32ToUint8Array(s.description.toUTF8().length), r),
              (r += 4),
              e.set(s.description.toUTF8(), r),
              (r += s.description.toUTF8().length),
              e.set(this._uint32ToUint8Array(s.width), r),
              (r += 4),
              e.set(this._uint32ToUint8Array(s.height), r),
              (r += 4),
              e.set(this._uint32ToUint8Array(s.colorDepth), r),
              (r += 4),
              e.set(this._uint32ToUint8Array(s.colorNumber), r),
              (r += 4),
              e.set(this._uint32ToUint8Array(s.data.length), r),
              (r += 4),
              e.set(s.data, r);
        }
        return e;
      ...
      serializeMetadata() {
        const t =
            4 +
            this.metadata.blocks.reduce((t, e) => t + 4 + e.serializedSize, 0) +
            (this.arrayBuffer.byteLength > this.metadata.framesOffset
              ? this.arrayBuffer.byteLength - this.metadata.framesOffset
              : 0),
          e = new Uint8Array(t);
        e.set(this.metadata.signature.toUTF8());
        let s = 4,
          r = !1;
        return (
          this.metadata.blocks.forEach((t, a, i) => {
            i.length - 1 === a && (r = !0),
              (e[s] = t.blockTypeNubmer | (r << 7)),
              (s += 1),
              e.set(this._uint24ToUint8Array(t.serializedSize), s),
              (s += 3),
              e.set(this._serializeMetadataBlock(t), s),
              (s += t.serializedSize);
          }),
          e.set(
            new Uint8Array(this.arrayBuffer).subarray(
              this.metadata.framesOffset
            ),
            s
          ),
          (this.arrayBuffer = e.buffer),
          this
        );
      }
      _parseMetadataBlock(t, e, s, r) {
        const a = t.subarray(e, e + r);
        let i = 0;
        const n = new FLACMetadataBlockData();
        switch (s) {
          case "STREAMINFO":
            (n.minBlockSize = this._getBytesAsNumber(a, i, 2)),
              (i += 2),
              (n.maxBlockSize = this._getBytesAsNumber(a, i, 2)),
              (i += 2),
              (n.minFrameSize = this._getBytesAsNumber(a, i, 3)),
              (i += 3),
              (n.maxFrameSize = this._getBytesAsNumber(a, i, 3)),
              (i += 3),
              (n.sampleRate = this._getBytesAsNumber(a, i, 3) >> 4),
              (i += 2),
              (n.numberOfChannels = 1 + ((a[i] >> 1) & 7)),
              (n.bitsPerSample = 1 + ((1 & a[i]) << 4) + (a[i + 1] >> 4)),
              (i += 1),
              (n.totalSamples =
                (15 & a[i]) * Math.pow(2, 32) +
                this._getBytesAsNumber(a, i + 1, 4)),
              (i += 5),
              (n.rawMD5 = this._getBytesAsHexString(a, i, 16).toUpperCase());
            break;
          case "PADDING":
            break;
          case "APPLICATION":
            (n.applicationID = this._getBytesAsUTF8String(a, i, 4)),
              (i += 4),
              (n.applicationData = a.subarray(i));
            break;
          case "SEEKTABLE":
            (n.pointCount = r / 18), (n.points = []);
            for (let t = 0; t < n.pointCount; t++)
              n.points.push({
                sampleNumber: this._getBytesAsHexString(a, i, 8),
                offset: this._getBytesAsHexString(a, i + 8, 8),
                numberOfSamples: this._getBytesAsHexString(a, i + 16, 2)
              }),
                (i += 18);
            break;
          case "VORBIS_COMMENT":
            const t = this._getBytesAsNumberLittleEndian(a, i, 4);
            (i += 4),
              (n.vendorString = this._getBytesAsUTF8String(a, i, t)),
              (i += t);
            const e = this._getBytesAsNumberLittleEndian(a, i, 4);
            (i += 4), (n.comments = new VorbisCommentPacket());
            let o = 0;
            for (let t = 0; t < e; t++)
              (o = this._getBytesAsNumberLittleEndian(a, i, 4)),
                (i += 4),
                n.comments._addComment(this._getBytesAsUTF8String(a, i, o)),
                (i += o);
            break;
          case "CUESHEET":
            break;
          case "PICTURE":
            (n.APICtype = this._getBytesAsNumber(a, i, 4)), (i += 4);
            const h = this._getBytesAsNumber(a, i, 4);
            (i += 4),
              (n.MIMEType = this._getBytesAsUTF8String(a, i, h)),
              (i += h);
            const c = this._getBytesAsNumber(a, i, 4);
            (i += 4),
              (n.description = this._getBytesAsUTF8String(a, i, c)),
              (i += c),
              (n.width = this._getBytesAsNumber(a, i, 4)),
              (i += 4),
              (n.height = this._getBytesAsNumber(a, i, 4)),
              (i += 4),
              (n.colorDepth = this._getBytesAsNumber(a, i, 4)),
              (i += 4),
              (n.colorNumber = this._getBytesAsNumber(a, i, 4)),
              (i += 4);
            const m = this._getBytesAsNumber(a, i, 4);
            (i += 4), (n.data = a.subarray(i, i + m));
        }
        return n;
      }
      _parseMetadata() {
        const t = new Uint8Array(this.arrayBuffer);
        this.metadata.signature = this._getBytesAsUTF8String(t, 0, 4);
        let e,
          s = 4,
          r = !1,
          a = 0;
        for (; !r && s < t.length; ) {
          if (a++ > 42)
            throw new RangeError(
              "Too much METADATA_BLOCKS. Looks like file corrupted"
            );
          ((e = new FLACMetadataBlock()).offset = s),
            (r = !!(t[s] >> 7)),
            (e.blockTypeNubmer = 127 & t[s]),
            (e.blockType = this._getBlockType(e.blockTypeNubmer)),
            (s += 1),
            (e.blockSize = this._getBytesAsNumber(t, s, 3)),
            (s += 3),
            (e.data = this._parseMetadataBlock(t, s, e.blockType, e.blockSize)),
            (s += e.blockSize),
            this.metadata.blocks.push(e);
        }
        return (this.metadata.framesOffset = s), this;
      }
    };
  })();
  /* jshint    ignore: end */
  ...
  this.onmessage = ({ data: data }) => {
    ...
    function niceXhr(config) {
      ...
      xhr.onload = e => {
        if (200 === e.target.status) config.onSuccess(e.target.response);
        else console.warn("Error with getting data from", e.target.responseURL);
        config.anyway();
      };
    ...
    niceXhr({
      ...
      onProgress: e => {
        ...
        if (e.lengthComputable) {
          ...
          postMessage([
            trackData,
            `^downloading ${loaded}/${total}^mb : ${percent}%`
          ]);
        } else {
      ...
      onSuccess: response => {
        ...
        for (let i = 0; i < L; i += 6144) {
          if (i % (6144 * 20) === 6144 * 19) {
            // let it display state at every 120K
        ...
        const isMP3 =
          ...
          "ID3" === binArr2utfStr(data.subarray(0, 3));
        const isFLAC = "fLaC" === binArr2utfStr(data.subarray(0, 4));

        if (!(isMP3 || isFLAC))
          return console.warn("Decrypted file not MP3 nor FLAC!", data);
        ...
        if (isMP3) {
          ...
          if (albumData.label) writer.setFrame("TPUB", albumData.label);
        ...
        if (isFLAC) {
          ...
          tags._artistsArray.forEach(artist =>
            writer.addComment("ARTIST", artist)
          );
          if (tags.track) writer.addComment("TRACKNUMBER", tags.track);
          ...
          if (azLyrics.lyrics) writer.addComment("LYRICS", azLyrics.lyrics);
          // if (azLyrics.year)
        ...
        niceXhr({
          ...
          anyway: () => {
            if (isMP3) writer.addTag();
            if (isFLAC) writer.serializeMetadata();
...
function azLyrics(artist, title, onsuccess, anyway) {
  ...
  const data = {
    ...
    url: url,
    onload: e => {
      if (200 === e.status) onsuccess(parseAzLyricsPage(e.response));
      else console.warn("Cannot grab lyrics from", e.finalUrl);
      anyway();
    },
    onerror: anyway,
    onabort: anyway
...
function formatAzLyricsUrl(artist, title) {
  const partOfUrl = [artist, title]
    .map(el => el.toLowerCase().replace(/[^\w]/g, ""))
    .join("/")
    .replace(/^the/, "");
...
function zeroPad(data) {
  const aesBS = 16;
  const l = data.length;
  if (l % aesBS !== 0) data += "\0".repeat(aesBS - (l % aesBS));
  return data;
}
...
function encryptURL(track, fmt) {
  ...
  let steps = [track.MD5_ORIGIN, fmt, track.SNG_ID, track.MEDIA_VERSION].join(
    urlsep
  );
...
if (!localStorage.dlAzLyrics) unsafeWindow.localStorage.dlAzLyrics = true;
...
function dzTags(trackInfo) {
  ...
  const artistsArray =
    trackInfo.ARTISTS && GM_info.scriptHandler !== "Greasemonkey"
      ? [...new Set(trackInfo.ARTISTS.map(a => a.ART_NAME))]
      : [trackInfo.ART_NAME];
  return {
...
function dzDownload(track, trackId, fmt) {
  ...
  function processTrack() {
    mainWk.postMessage({
      url: `https://e-cdns-proxy-${
        track.MD5_ORIGIN[0]
      }.dzcdn.net/mobile/1/${encryptURL(track, fmt)}`,
...
function messageProcessor(msg) {
  ...
  if (!trackEl) console.warn("Downloaded track not found in track list!");
...
function getFilesize(track, fmt_name) {
  ...
  let size = 0;
  if (track[sizeProp] && !isNaN(track[sizeProp])) size = +track[sizeProp];
  return size;
}
...
function generateDlTrack(track) {
  ...
  const infosTooltips =
    ...
    tags.title +
    "\n^album:\t" +
    tags.album + // disk number sometimes not recognized when change track playing ????
    "\n^duration:\t" +
    tags.time;
  ...
  if (showMp3_32)
    if (getFilesize(track, "MP3_32") > 0)
      linksEl.appendChild(
        generateDlLink(track, "mp3 32", fmtMp3_32, getFilesize(track, "MP3_32"))
      );
  if (showMp3_64)
    if (getFilesize(track, "MP3_64") > 0)
      linksEl.appendChild(
        generateDlLink(track, "mp3 64", fmtMp3_64, getFilesize(track, "MP3_64"))
      );
  if (showMp3_128)
    ...
    if (getFilesize(track, "MP3_320") > 0)
      linksEl.appendChild(
        generateDlLink(
          ...
          "mp3 320",
          fmtMp3_320,
          getFilesize(track, "MP3_320")
        )
      );
  if (showFLAC)
    if (getFilesize(track, "FLAC") > 0)
      linksEl.appendChild(
        generateDlLink(track, "flac", fmtFLAC, getFilesize(track, "FLAC"))
      );
...
function isLoggedIn() {
  return (
    typeof unsafeWindow.USER !== "undefined" && unsafeWindow.USER.USER_ID !== 0
  );
}

function getTrackListByHand() {
  const links = Array.from(
    document.querySelectorAll("[data-instance=track_name]")
  );
  ...
  for (let i = 0; i < links.length; i++) {
    Object.keys(trackDB).forEach(trackId => {
      if (trackDB[trackId].ALB_ID === albums[i])
        if (
          trackDB[trackId].SNG_TITLE === names[i] ||
          `${trackDB[trackId].SNG_TITLE} ${trackDB[trackId].VERSION}` ===
            names[i]
        )
...
function getTrackListIndexByHand() {
  ...
  if (currentEl) currentIndex = names.indexOf(currentEl.textContent);
  return currentIndex;
}
...
function refreshTracklist() {
  ...
  if (unsafeWindow.dzPlayer) {
    ...
    if (DEBUG)
      console.info("Tracks are getting manually. Be careful with result");
    // Do some magic
  ...
  list.forEach((track, index) => {
    ...
    if (index === current) trackDiv.classList.add("current");
...
function addInsertVariant(value) {
  ...
  row.addEventListener("click", () => {
    ...
    elNameProto.value =
      src.substr(0, elNameProto.selectionStart) +
      "%" +
      value +
      src.substr(elNameProto.selectionEnd);
    if (sel.start === src.length) {
      elNameProto.selectionStart = elNameProto.selectionEnd =
        elNameProto.value.length;
    } else {
...
if (forceAzLyrics) unsafeWindow.localStorage.dlAzLyrics = forceAzValue;
...
if (showAzLyrics || showListDownloader) rootEl.appendChild(elTrackListHeader);
...
function hidePannel() {
  if (!rootEl.classList.contains("dzHidden")) triggerEl.click();
}
...
addEventListener("keydown", e => {
  switch (e.code) {
    case "Escape":
      ...
      break;
    case "KeyD":
      if (document.activeElement.nodeName !== "INPUT") triggerEl.click();
      break;
    default:
...
const delayStyleElement = document.head.appendChild(
...
function onInterfaceLoaded() {
  ...
  if (openOnStart) triggerEl.click();
}

const waitInterfaceLoading = () => {
  if (!document.querySelector("button.control-qlist"))
    return setTimeout(waitInterfaceLoading, 150);
  onInterfaceLoaded();
};
